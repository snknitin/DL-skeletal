...
from src.data.components.sta import StateSpace
...
class MultiSKUEnvironment(gym.Env):
    ...
    def load_data(self):
        ...
        self.pl_ratio_table = pd.read_pickle(self.data_dir / 'Placement_Ratio_table.pkl')
    ...
    def setup_state_space(self):
        ...
        self.inventory_state = StateSpace(num_fcs=self.num_fc, lt_values=lead_times, rp_arrays=rp_arrays,
                                          forecast_horizon=self.forecast_horizon)

    def setup_action_space(self):
        self.action_space = spaces.Box(low=0, high=4, shape=(self.num_fc, self.num_sku), dtype=np.float32)
    ...
    def step(self, action):
        ...
        self.inventory_state.update(sales=sales_at_FC.flatten(), actions=action.flatten(),
                                    multiplier=self.multiplier.flatten())
