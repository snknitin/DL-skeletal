...
class MultiFCEnvironment(gym.Env):
    ...
    def calculate_reward_OT(self, inventory, realized_demand, mapped_dem_pr, benefits_data_table, compute_sla):
        ...
        for i in range(temp_inventory.shape[1]):
            ...
            ### SLA calculations not yet committed
            benefits_data_table_array = np.array(benefits_data_table)
            M = 1 - self.min_max_scaler.fit_transform(benefits_data_table_array)
            ...
            if compute_sla:
                ...
                M = 1.0 - ((benefits_data_table_array - min_val) / (max_val - min_val))
                alpha, beta = 0.1, 1000
            ...
            uot = ot.solve(M.cpu().numpy() / uot_scale_factor.item(), supply.astype('float'), demand.astype('float'),
                           method='emd')
